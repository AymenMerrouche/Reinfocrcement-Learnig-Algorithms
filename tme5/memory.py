...
class Buffer:
    """
    buffer customisé pour le tme actor critic
    pas de taille max
    ...
    def sample(self):
        ...
        liste_etats, liste_actions, liste_rewards, liste_new_etats, liste_done = [], [], [], [], []

        for (s,a,r,s_prime,done, info) in self.buffer:
        ...
        s_tensor, a_tensor, r_tensor, s_prime_tensor, done_tensor = torch.tensor(liste_etats, dtype=torch.float), torch.tensor(liste_actions), \
                                                               torch.tensor(liste_rewards, dtype=torch.float), torch.tensor(liste_new_etats, dtype=torch.float), \
                                                               torch.tensor(liste_done, dtype=torch.float)
