...
class PPO_kl(nn.Module):
    def __init__(self, env, opt, alpha, gamma, lmbda, K, delta, entropy_weight):
    ...
    def train(self):
        ...
        for i in range(self.K):
            ...
            entropy = self.compute_entropy(probas)
            ...
            loss = -L_theta + self.beta * kl + F.smooth_l1_loss(self.V_phi(s) , td_0.detach().float()) + self.entropy_weight * entropy
...
if __name__ == '__main__':
    ...
    agent = PPO_kl(env, config, config["alpha"], config["gamma"], config["lmbda"], config["K"], config["delta"], config["entropy_weight"] )
