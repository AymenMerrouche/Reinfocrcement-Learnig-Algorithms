...
class PPO_clipped_entropy(nn.Module):
    def __init__(self, env, opt, alpha, gamma, lmbda, K, epsilon, entropy_weight ):
        super(PPO_clipped_entropy, self).__init__()
        ...
        #
        self.entropy_weight = entropy_weight # <==================== difference par rapport a ppo_clipped
    ...
    def compute_entropy(self, probas):
        ...
        """ fonction qui calcul l'entropie d'un tensor de probas batch x nb actions """
        entropy = torch.distributions.Categorical(probas).entropy()
    ...
    def train(self):
        ...
        for i in range(self.K):
            ...
            #                                                     entropy computation
            # =======================================================================
            entropy = self.compute_entropy(pi)
            # loss combinés des deux v_phi et pi_theta
            # float sur td_0 a cause de lunar bizarement il renvoit float64
            # - entropy dans la loss
            loss = -torch.min(ratio * avantages, torch.clamp(ratio, 1-self.epsilon, 1+self.epsilon) * avantages) +  self.entropy_weight * entropy + F.smooth_l1_loss(self.V_phi(s) , td_0.detach().float())
            #optimization
...
if __name__ == '__main__':
    ...
    outdir = "./XP/" + config["env"] + f"/PPO_clipped_entropy_ceiling/"+ tstart
    ...
    agent = PPO_clipped_entropy(env, config, config["alpha"], config["gamma"], config["lmbda"], config["K"], config["epsilon"],config["entropy_weight"] )
        ...
        while not done:
            ...
            for t in range(t_step):
                ...
                if done:
                    ...
                    rsum = 0
                    break
            #if score < 500: # test si c'est meilleur de pas apprendre a ce point la
            agent.train()
    env.close()
