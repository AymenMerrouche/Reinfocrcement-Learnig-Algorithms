...
class mu(nn.Module):
    """
    classe pour Mu reseau a une couche cachée qui change selon l'environement
    """
    def __init__(self):
        super(mu, self).__init__()
        # 8 pour lunar
        # je l'ai changé a la main
        self.linear1 = nn.Linear(8, 400)
        self.linear2 = nn.Linear(400, 300)
        self.linear_final = nn.Linear(300, 2)

    def forward(self, x):
        x = F.relu(self.linear1(x))
        x = F.relu(self.linear2(x))
        # on multiple par 2 pour pendulum
        mu = torch.tanh(self.linear_final(x))
        return mu

class Q(nn.Module):
    """
    classe pour Q reseau a deux couches cachées qui change selon l'environement
    """
    def __init__(self):
        super(Q, self).__init__()
        # 8 pour lunar continuous
        self.linear_state = nn.Linear(8, 200)
        self.linear_action = nn.Linear(2,200)
        self.linear_Q = nn.Linear(400, 300)
        self.linear_final = nn.Linear(300,1)

    def forward(self, x, a):
        h1 = F.relu(self.linear_state(x))
        h2 = F.relu(self.linear_action(a.view(128,-1)))
        cat = torch.cat([h1,h2], dim=1)
        q = F.relu(self.linear_Q(cat))
