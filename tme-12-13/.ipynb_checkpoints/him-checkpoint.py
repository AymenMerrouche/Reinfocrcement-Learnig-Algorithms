...
class GAILAgent(nn.Module):
    def __init__(self, env, opt):
        ...
        self.v = nn.Sequential(nn.Linear(nb_features, h), nn.ReLU(), nn.Linear(h, 1)).to(device)
        ...
        # ---expert---#
        with open("./expert_data/expert.pkl", "rb") as handle:
        ...
        # ---device---#
        self.longTensor = torch.empty(1, dtype=int, device="cuda")
        self.floatTensor = torch.empty(1, dtype=float, device="cuda")
        self.to(dtype=float, device="cuda")
    ...
    def train(self, beta):
        ...
        x_agent = x_agent.to(dtype=float).to(device)
        one = torch.ones_like(x_expert).to(device)
